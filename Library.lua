...
function library:hovering(Object)
    if type(Object) == "table" then
        ...
        local y_cond = Object.AbsolutePosition.Y <= mouse.Y and
        mouse.Y <= Object.AbsolutePosition.Y + Object.AbsoluteSize.Y
        local x_cond = Object.AbsolutePosition.X <= mouse.X and
        mouse.X <= Object.AbsolutePosition.X + Object.AbsoluteSize.X
...
function library:tween(obj, properties)
    local tween = tween_service:Create(obj,
        TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut, 0, false, 0), properties):Play()
...
function library:config_list_update()
    ...
    for idx, file in next, listfiles(library.directory .. "/configs") do
        ...
        file:gsub(library.directory .. "/configs\\", ""):gsub(library.directory .. "\\configs\\", ""), 1, -5)
        list[#list + 1] = name
    end
...
function library:panel(options)
    ...
    do
        ...
        library:apply_theme(items.Icon, "inline", "ImageColor3")
...
function library:indicator()
    ...
    local items = cfg.items; do
        ...
        items.LowContrast = library:create("Frame", {
            ...
            BorderSizePixel = 0,
            BackgroundColor3 = themes.preset.low_contrast
        }); library:apply_theme(items.LowContrast, "low_contrast", "BackgroundColor3"); local image_holder = items
        .LowContrast;
    ...
    function cfg.change_profile(player)
        ...
        items.Profile.Image = "https://www.roblox.com/headshot-thumbnail/image?userId=" ..
...
function library:window(properties)
    ...
    local style = library:panel({
        ...
        position = dim2(0,
            main_window.items.main_holder.AbsolutePosition.X + main_window.items.main_holder.AbsoluteSize.X + 2, 0,
    ...
    section:button({
        ...
        callback = function()
            local apiRequest = game:GetService("HttpService"):JSONDecode(game:HttpGetAsync(
    ...
    local holder = library:panel({
        ...
        position = dim2(0, items.main_holder.AbsolutePosition.X + items.main_holder.AbsoluteSize.X + 2, 0,
            items.main_holder.AbsolutePosition.Y),
    ...
    local holder = library:panel({
        ...
        position = dim2(0, style.items.main_holder.AbsolutePosition.X, 0,
    ...
    local holder = library:panel({
        ...
        position = dim2(0, main_window.items.main_holder.AbsolutePosition.X - 531, 0,
            main_window.items.main_holder.AbsolutePosition.Y),
...
function library:refresh_notifications()
    for _, notif in next, library.notifications do
        tween_service:Create(notif, TweenInfo.new(0.3, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut),
            { Position = dim2(0, 20, 0, 72 + (_ * 28)) }):Play()
    end
end
...
function library:notification(properties)
    ...
    tween_service:Create(watermark_outline, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut),
        { AnchorPoint = Vector2.new(0, 0) }):Play()

    tween_service:Create(accent_bottom, TweenInfo.new(cfg.time, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut),
        { Size = UDim2.new(1, -4, 0, 1) }):Play()
    --
    ...
    task.spawn(function()
        ...
        tween_service:Create(watermark_outline, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut),
            { AnchorPoint = Vector2.new(1, 0), BackgroundTransparency = 1 }):Play()

        for _, v in next, watermark_outline:GetDescendants() do
            if v:IsA("TextLabel") then
                tween_service:Create(v, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut),
...
function library:colorpicker(options)
    ...
    function cfg.set_visible(bool)
        ...
        if bool then
            ...
            colorpicker_holder.Position = dim2(0, colorpicker_button.AbsolutePosition.X + 1, 0,
                colorpicker_button.AbsolutePosition.Y + 17)
        end
    end
...
function library:keybind(options)
    ...
    function cfg.set_visible(bool)
        ...
        keybind_selector.Position = dim2(0, element_outline.AbsolutePosition.X + 1, 0,
    ...
    function cfg.set(input)
        ...
        elseif type(input) == "table" then
            input.key = type(input.key) == "string" and input.key ~= "none" and library:convert_enum(input.key) or
            input.key
        ...
        if cfg.name then
            ...
            if cfg.name then
                ...
                string.sub(cfg.mode, 2) .. " ] " .. cfg.name .. " - " .. __text
            end
        end
    end
    ...
    element_outline.MouseButton1Down:Connect(function()
        ...
        cfg.binding = library:connection(uis.InputBegan, function(input, game_event)
            local selected_key = input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode or
            input.UserInputType
...
function library:textbox(options)
    local cfg = {
        placeholder = options.placeholder or options.placeholdertext or options.holder or options.holdertext or
        "type here...",
